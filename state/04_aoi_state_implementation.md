...
1. Should the state system live **inside** `TestAutomationSDK` (shared), **inside** `AOI_Main` (local), or in a **new project** `AOI_Main.StateEngine`?  
   Recommendation: new project `AOI_Main.StateEngine` — keeps infrastructure separate, testable in isolation.
...
```xml
<!-- AOI_Main.StateEngine.csproj -->
<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
    <TargetFramework>net48</TargetFramework>
    <LangVersion>7.3</LangVersion>
    <AssemblyName>AOI_Main.StateEngine</AssemblyName>
    <RootNamespace>Camtek.AOIMain.StateEngine</RootNamespace>
...
File: `AOI_Main.StateEngine.Tests/AoiEventAggregatorTests.cs`  
...
File: `AOI_Main.StateEngine.Tests/BridgeOrchestratorTests.cs`
...
## Phase 4 — Integration into AOI_Main

### 4.1 Bootstrapper / Composition Root
Show exactly where in `AOI_Main`'s entry point the state engine is wired up:

```csharp
// In AOI_Main startup / test setup
...
// In AOI_Main test steps — consume state
...
5. **Phase 4** — wire into AOI_Main entry point; run smoke test on real machine
