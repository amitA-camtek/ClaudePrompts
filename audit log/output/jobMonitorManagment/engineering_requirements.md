...
| ID | Priority | Requirement |
|---|---|---|
| API-001 | M | The service **shall** expose a read-only HTTP API on a configurable port (default **5100**) using ASP.NET Core Kestrel, hosted within the same Windows Service process. |
...
| ID | Priority | Requirement |
|---|---|---|
| INS-001 | M | An `install.ps1` script **shall** register the service, create `C:\bis\auditlog\`, and copy the default `FileClassificationRules.json` to that path if it does not already exist. |
